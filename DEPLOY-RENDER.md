...
若要在小红书等渠道「一码一用」售卖，可启用内置序列号校验，仍使用 **Static Site**，无需单独部署后端。

**机制简述**：  
- 有效序列号以**哈希**形式写在构建里（`code-hashes.json`），前端用浏览器本地校验；明文序列号只保存在你本机的 `codes.txt`（已加入 .gitignore），用于你发给下单用户。  
- **一码一张贺卡**：每个序列号在「成功封存并生成一张贺卡」后才被标记为已用；同一设备不能再用该码进入，换设备也会提示已使用。用户关闭页面后再打开，需要新码才能再制。

**数量**：成百上千用户就生成成百上千个码，例如 `node scripts/generate-codes.js 1000`。序列号可分批生成并追加到 `codes.txt` 和 `code-hashes.json`。
...
1. **生成一批序列号**（在项目根目录执行）  
   ...
   npm run codes
   ```
   默认生成 50 个。成百上千用户可一次多生成，例如：  
   ```bash
   node scripts/generate-codes.js 1000
   ```
   - 会生成/追加 **codes.txt**（明文序列号，仅自己保存，勿提交 Git），发给下单用户用。  
   - 会更新 **code-hashes.json**（哈希列表，需提交并重新部署后，前端才会校验这批新码）。

2. **提交并部署**  
   - 把 `code-hashes.json` 的改动提交并 push，触发 Render 重新构建。  
   - 部署完成后，访问站点会先进入「输入序列号」页，验证通过后才能进入定制页。

3. **发码给买家**  
   - 从本机 `codes.txt` 里复制一个未发过的序列号，通过小红书私信/自动回复/商品卡密等方式发给下单用户即可。  
   - 每发掉一个就做下标记或从文件里删掉，避免重复发放。

4. **不启用序列号时**  
   - 若 `code-hashes.json` 内容为 `[]`（空数组），站点不会显示序列号页，所有人可直接进入定制页。  
   - 若之后想停用序列号，把 `code-hashes.json` 改回 `[]` 并重新部署即可。
